...
| Rule | Status |
| --- | --- |
| **Hide the horizontal header** if selection lags | **The single biggest lever.** Measured 3.117 s → 0.003 s on `selectAll()` at 200k rows, `flags()` calls 800,287 → 303 — about 1000×. QTBUG-59478, open since 2017, and its fix was *abandoned* in 2017. If the header must stay, subclass `QHeaderView` and override `initStyleOptionForIndex` |
...
| Cache the `flags()` return value | **Keep, restated.** Measured ~1.3× (3.47 s → 2.55 s at 200k), not 20×. The ADR's "7.760 s → 0.396 s" conflated time-inside-`flags()` with an operation total. It is one line and still worth it — but note the 800k calls it optimises are *caused* by the header bug above. Fix the cause first |
...
| Signal and state machinery | Native menu placement — screenshot job only |

The screenshot job is `workflow_dispatch`-only: a documentation tool, not a
gate. It renders each screen against a fixture capture in both schemes, and
uploads with `if-no-files-found: error` so a silent capture failure fails the
run rather than producing an empty artifact nobody notices.
